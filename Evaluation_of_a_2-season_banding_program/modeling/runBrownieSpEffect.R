runBrownieSpEffect <- function(mergeddata, pooledHybrids = T, jagsmodel,runmode=NULL){
  require(runjags)
  ...
  nyrs <- 9
 ### mod.parms <- c( 'fit.sim.stat','fit.obs.stat') # Add ftstat - Bayesian p-value difference in chi-squared values
  	###	fstat.am[s,i,i] <- ((exp.am[s,i,i]-recovmat.am[s,i,i])^2)/(exp.am[s,i,i]+0.5)

if(runmode == "gof"){
 mod.parms<- c('fit.obs.stat', 'fstat.sim.am', 'fstat.am','exp.am', 'fstat.sim.af', 'fstat.af', 'exp.af',
                               'fstatP.sim.am', 'fstatP.am', 'fstatP.sim.af', 'fstatP.af' , 
                               'fstat.sim.jm', 'fstat.jm',   'fstatP.sim.jm', 'fstatP.jm' ,
                               'fstat.sim.jf', 'fstat.jf',   'fstatP.sim.jf', 'fstatP.jf' )
   }else{
 mod.parms<- c('S.am', 'S.af','S.jm','S.jf','SH.am','SN.am','SH.af','SN.af','SH.jm',
               'SN.jm','SH.jf','SN.jf','f.jm','f.jf','f.am','f.af')
 }
...
jags.data <-list(recovmat.am=recovmat.am, 
  ...
  mod.inits <- function (){list(f.am=matrix(runif(nyrs*3,0.05,0.2),nrow=3, ncol=nyrs),f.af=matrix(runif(nyrs*3,0.05,0.2),nrow=3, ncol=nyrs),f.jm=matrix(runif(nyrs*3,0.05,0.2),nrow=3, ncol=nyrs),f.jf=matrix(runif(nyrs*3,0.05,0.2),nrow=3, ncol=nyrs),
                                muH.am=matrix(rnorm((nyrs-1)*3,0,1.5),nrow=3, ncol=nyrs-1), muN.am=matrix(rnorm((nyrs-1)*3,0,1.5),nrow=3, ncol=nyrs-1),
                                muH.af=matrix(rnorm((nyrs-1)*3,0,1.5),nrow=3, ncol=nyrs-1), muN.af=matrix(rnorm((nyrs-1)*3,0,1.5),nrow=3, ncol=nyrs-1),
                                muH.jm=matrix(rnorm((nyrs-1)*3,0,1.5),nrow=3, ncol=nyrs-1),
                                muH.jf=matrix(rnorm((nyrs-1)*3,0,1.5),nrow=3, ncol=nyrs-1)
  )} 
  
  ## jagsUI version of model run
  #ni <- 205000; nb <- 200000; nc <- 3 # 5000;2;2000;3
  #jagsui.out <- jags(jags.data, inits = mod.inits, mod.parms, "AgeSex_2season_annS.txt", n.chains=nc, n.iter=ni, n.burn=nb, parallel = TRUE, n.cores = 3)
  
  # Create data summary like below
  
  #sim.sum <- data.frame(row.names=rownames(jagsui.out$summary), 
  #                      Lower95=jagsui.out$summary[,3], 
  #                      Median=jagsui.out$summary[,5],
  #                      Upper95=jagsui.out$summary[,7],
  #                      Mean=jagsui.out$summary[,1],
  #                      SD=jagsui.out$summary[,2])
  
  # Get rid of deviance row
  #sim.sum <- sim.sum[-133,]
  
  ## ^ Note that this doesn't have any of the MCerror or AC summaries. Is that an issue?
  
  ## runjags version of model run
  
  #runjags.options("jagspath" = "C:/WorkSpace/Jags/x64/bin/jags-terminal.exe")
  #runjags.getOptions("jagspath" = "C:/WorkSpace/Jags/Bin/")
  #setwd('C:/Users/gzimmerman/Documents/Black Duck/TwoSeason/')
  #beg.time<-Sys.time()
  #spEffectmodel <- paste(readLines("jagsfiles/AgeSex_2season_annS_spEffect.txt"),collapse="\n")
  ...
  runjags.out<- jagsUI( model ="AgeSex_2season_annS_spEffect_Small.txt", parameters.to.save  = mod.parms,
                          data = jags.data, n.chains = 3, n.thin=6, inits = mod.inits, n.burnin = 100000, n.iter = 160000, 
                         parallel=TRUE)
  ...
  return(list(runjags.out = runjags.out))
}
